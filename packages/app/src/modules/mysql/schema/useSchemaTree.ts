...
const ACTIVE_EVENT = "mysql:active-changed";

/** Code MySQL surfaces for query-interrupted / KILL QUERY. */
const MYSQL_CANCEL_CODE = "70100";

function isMysqlTimeout(err: AppError): boolean {
  return err.kind === "Mysql" && (err as unknown as Record<string, unknown>)["mysql"] !== undefined
    ? ((err as unknown as { mysql?: { code?: string | null } }).mysql?.code ?? null) === MYSQL_CANCEL_CODE
    : false;
}

// Simpler check by inspecting message for 70100 code since AppError shape may vary
function isCancelError(err: AppError): boolean {
  return (
    (err as unknown as Record<string, unknown>)["code"] === MYSQL_CANCEL_CODE ||
    isMysqlTimeout(err) ||
    err.message?.includes("70100") ||
    false
  );
}
...
function buildSeededObjects(
  ...
  for (const s of schemas) {
    const relations = mysqlSchemaCache.getRelations(connectionId, s.name);
    if (relations) {
      objects.set(s.name, {
...
export interface UseSchemaTreeResult {
  ...
  /** Re-run `relations` fetch, using the auto-retry-on-70100 path. */
...
export function useSchemaTree(connectionId: string | null): UseSchemaTreeResult {
  ...
  useEffect(() => {
    ...
    const cachedSchemas = mysqlSchemaCache.getSchemas(connectionId);
    const hasCache = cachedSchemas.length > 0;
    const stale = isStale(mysqlSchemaCache.getSchemasFetchedAt(connectionId));
    ...
    schemaApi
      ...
      .then((schemas) => {
        if (cancelled) return;
        mysqlSchemaCache.recordSchemas(connectionId, schemas);
      ...
      .catch((e: unknown) => {
        ...
        if (stale && hasCache) {
          console.warn(
            "[argus.mysql.schema] background listSchemas refresh failed; keeping cache for",
            connectionId,
            e,
          );
          return;
        ...
        console.error("[argus.mysql.schema] listSchemas failed for", connectionId, err);
  ...
  useEffect(() => {
    ...
    return subscribeMysqlSchemaEvent((e) => {
      if (e.type === "invalidate" && e.connectionId === connectionId) {
        if (connectionId) mysqlSchemaCache.invalidate(connectionId);
        dispatch({ type: "invalidate" });
      }
    });
  }, [connectionId]);
  ...
  /**
   * Eager `relations` fetch with one auto-retry on MySQL code 70100.
   * - First call on idle: runs normally.
   * - If first call returns 70100: auto-retries once (retrying state).
   * - If retry also fails: surfaces error state with manual retry.
   * - listStructure / listTableExtras: NO auto-retry.
   */
  const runFetchRelations = useCallback(
    async (schema: string, isRetry: boolean) => {
      ...
      try {
        ...
        mysqlSchemaCache.recordRelations(connectionId, schema, payload);
        dispatch({ type: "relationsLoaded", schema, payload });
      } catch (e) {
        const err = e instanceof AppError ? e : new AppError("Internal", String(e));
        // Auto-retry on 70100, exactly once.
        if (!isRetry && isCancelError(err)) {
          console.warn(
            "[argus.mysql.schema] listRelations timed out (70100) — auto-retrying once for",
            connectionId,
            "/",
            schema,
          );
        ...
        console.error(
          "[argus.mysql.schema] listRelations failed for",
          connectionId,
          "/",
          schema,
          err,
  ...
  const runFetchStructure = useCallback(
    async (schema: string) => {
      ...
      try {
        ...
        const err = e instanceof AppError ? e : new AppError("Internal", String(e));
        console.error(
          "[argus.mysql.schema] listStructure failed for",
          connectionId,
          "/",
          schema,
          err,
  ...
  const runFetchTableExtras = useCallback(
    async (schema: string, relation: string) => {
      ...
      try {
        ...
        const err = e instanceof AppError ? e : new AppError("Internal", String(e));
        console.error(
          "[argus.mysql.schema] listTableExtras failed for",
          connectionId,
          "/",
          schema,
          "/",
