...
function fromConnection(c: Connection, mode: Mode): FormState {
  const p = c.params as Partial<MysqlParams> & Record<string, unknown>;
  return {
    name: mode === "duplicate" ? `${c.name} (copy)` : c.name,
    host: typeof p.host === "string" ? p.host : "",
    port: typeof p.port === "number" ? String(p.port) : "3306",
...
function toParams(form: FormState): MysqlParams {
  return {
    host: form.host.trim(),
...
export function MysqlConnectionForm({
  ...
  async function handleParse() {
    setUrlError(null);
    try {
      const parsed = await mysqlApi.parseUrl(urlInput);
  ...
  async function handleTest() {
    ...
    try {
      ...
      const result: TestResult = await mysqlApi.testConnection(params, password ?? null);
      if (result.ok) {
        setTest({
          kind: "ok",
  ...
  async function handleSaveAndConnect() {
    ...
    if (!saved) return;
    try {
      await mysqlApi.connect(saved.id);
        ...
        <Dialog.Content className={styles.dialog}>
            ...
            <div className={styles.grid}>
              ...
              <div className={styles.field}>
                ...
                <select
                  ...
                  value={form.ssl_mode}
                  onChange={(e) => setField("ssl_mode", e.target.value as SslMode)}
                >
                  {SSL_MODES.map((m) => (
          ...
          {test.kind === "err" && (
            <div className={styles.testRow} data-kind="err">
              {test.error.message}
              {test.error.mysql?.code && (
                <span className={styles.testCode}>SQLSTATE {test.error.mysql.code}</span>
              )}
            </div>
          )}
          {test.kind === "loading" && (
