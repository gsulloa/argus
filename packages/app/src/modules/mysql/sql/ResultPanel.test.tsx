 ...
 * 6.1/6.4/6.6 for MySQL).
 *
 * Asserts the shared `<TruncationBanner />` renders the response's own
 * `row_cap` (not a hardcoded 10,000), names the MySQL `LIMIT` clause, is
 * absent when the result is not truncated, and drops the "Raise limit"
 * control when `row_cap_source` is `"hard_ceiling"`.
 */
...
describe("MySQL ResultPanel truncation banner", () => {
  ...
  it("names the MySQL LIMIT clause", () => {
    render(
      <ResultPanel
