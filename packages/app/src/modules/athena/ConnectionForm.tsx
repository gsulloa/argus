...
function fromConnection(c: Connection, modeKind: FormMode["kind"]): FormState {
  const p = c.params as Partial<AthenaParams> & Record<string, unknown>;
  return {
    name: modeKind === "duplicate" ? `${c.name} (copy)` : c.name,
    auth: (p.auth as AthenaAuth | undefined) ?? "access_keys",
    profile: typeof p.profile === "string" ? p.profile : "",
    region: typeof p.region === "string" ? p.region : "us-east-1",
    workgroup: typeof p.workgroup === "string" ? p.workgroup : "primary",
    outputLocation: typeof p.output_location === "string" ? p.output_location : "",
...
function getTitle(mode: FormMode): string {
  switch (mode.kind) {
    case "create":
      return "New Amazon Athena connection";
    case "edit":
      return "Edit Amazon Athena connection";
    case "duplicate":
      return "Duplicate Amazon Athena connection";
  }
}

export function AthenaConnectionForm({
  open,
  onOpenChange,
  mode,
  onSaved,
  ...
  function isFormValid(): boolean {
    ...
    if (!form.workgroup.trim()) return false;
  ...
  function buildParams(): AthenaParams {
    return {
      auth: form.auth,
      region: form.region,
      workgroup: form.workgroup.trim(),
      output_location: form.outputLocation.trim() || undefined,
      read_only: form.readOnly,
      profile: form.auth === "profile" ? form.profile : undefined,
    };
  }

  // Browse S3 buckets using the credentials currently entered in the form
  // (same resolution as Test / Connect). Requires valid credentials, so it is
  // gated on the same form validity as the Test button.
  async function handleBrowseBuckets() {
    ...
    try {
      ...
      const list = await athenaApi.listS3Buckets(buildParams(), secret);
      setBuckets(list);
      if (list.length === 0) {
        setBucketErr("No buckets visible with these credentials.");
      }
    } catch (e) {
      const err = toAppError(e);
      setBucketErr(err.aws?.message ?? err.message ?? "Failed to list buckets");
      setBuckets(null);
    } finally {
  ...
  function handlePickBucket(bucket: string) {
    if (!bucket) return;
    // Default to a dedicated results prefix; the user can edit it afterwards.
    setField("outputLocation", `s3://${bucket}/athena-results/`);
  }
  ...
  async function handleTest() {
    ...
    try {
      ...
      const result = await athenaApi.testConnection(params, secret);
      if (result.ok) {
    ...
    try {
      ...
      if (saved) {
        ...
        if (variant === "save-and-connect") {
          await athenaApi.connect(saved.id);
          ...
          <div className={styles.grid}>
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                onChange={(e) => setField("workgroup", e.target.value)}
                placeholder="primary"
              />
              {!form.workgroup.trim() && (
                <div className={styles.error}>Required</div>
            ...
            <div className={`${styles.field} ${styles.fieldFull}`}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "stretch" }}>
                <input
                  ...
                  onChange={(e) => setField("outputLocation", e.target.value)}
                  placeholder="s3://my-results-bucket/athena/"
                />
                <button
                  type="button"
                  className={styles.button}
                  onClick={() => void handleBrowseBuckets()}
                  disabled={!isFormValid() || bucketLoading}
                  title="List S3 buckets using the credentials above"
                >
                  {bucketLoading ? "Loading…" : "Browse buckets"}
                </button>
              </div>
              {buckets && buckets.length > 0 && (
                <select
                  className={styles.select}
                  value=""
                  onChange={(e) => handlePickBucket(e.target.value)}
                  style={{ marginTop: 6 }}
                >
              ...
              {bucketErr && <div className={styles.error}>{bucketErr}</div>}
              <span className={styles.hint}>
                Required if your workgroup does not enforce an output location.
                {!isFormValid() && " Enter credentials above to browse buckets."}
              </span>
            </div>
