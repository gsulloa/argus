/**
 * §26.3 — Frontend tests for MS SQL Server filter model helpers.
 * Tests pure logic functions in data/types.ts: isCompleteRow, modelToPayload.
 *
 * Mirrors src/modules/mysql/data/__tests__/filterHelpers.test.ts with MSSQL-
 * specific notes (no ILIKE; spatial/rowversion ops are disabled in the UI but
 * the pure helpers don't reject them — rejection is in the Rust layer).
 */
...
describe("isCompleteRow", () => {
  ...
  it("returns true for LIKE with a non-empty string value", () => {
    // T-SQL LIKE (case-sensitive by default; MSSQL backend handles LOWER() wrapping).
...
describe("EMPTY_FILTER_ROW", () => {
  it("has enabled=true and op=Contains by default", () => {
    ...
    // MSSQL uses Contains as the default operator (same as MySQL, no ILIKE).
