/**
 * MySQL FilterBar — same operator set as Postgres EXCEPT:
 * - No ILIKE / NOT ILIKE operators.
 * - Case-insensitive toggle on LIKE/Contains/StartsWith/EndsWith.
 *
 * The toggle maps to the `caseInsensitive: true` flag embedded in the
 * FilterValue as a wrapper object; the backend emits `LOWER(col) LIKE LOWER(?)`.
 *
 * For v1 this is a simplified bar with column select, operator select, and value input.
 */
...
function FilterRowEditor({ row, columns, onChange, onRemove }: RowEditorProps) {
  ...
  // Case-insensitive flag is stored in a synthetic string prefix for v1.
  // The backend reads the `caseInsensitive` field in the Condition struct.
  // For simplicity we pass it as a top-level field on the condition.
  // Since FilterRow doesn't have this field, we store it in a side channel
  // using a special value prefix convention recognized by the backend.
  // For v1 we just track it locally and pass the toggle value as-is.
  // TODO: wire caseInsensitive flag properly when backend shape is confirmed.
