/**
 * §20.1 — MySQL QueryEditor component.
 *
 * CodeMirror 6 editor configured for MySQL SQL. Mirrors the Postgres
 * QueryEditor but uses the MySQL dialect and backtick autocomplete sources.
 *
 * Key bindings:
 *   Mod-Enter      → onRun (current statement)
 *   Mod-Shift-Enter → onRunAll (all statements)
...
export interface ErrorMark {
  /** 0-based character offset (backend returns 1-based; callers must subtract 1). */
  from: number;
  /** Exclusive end. */
  to: number;
}
    ...
    useEffect(() => {
      ...
      const state = EditorState.create({
        ...
        extensions: [
          ...
          errorMarkField,
          langCompartment.current.of(sql({ dialect: MySQL })),
