...
Bootstrap procedure to wire up the Argus Beta release pipeline. Do this **once**. Every step here is something Claude/CI cannot do for you because it lives in external dashboards, your keychain, or 1Password.
...
6. Beta icon variant in src-tauri/icons-beta/    (visually distinct app)
7. GitHub Actions workflow permission toggle     (let the bot push commits)
```
...
Required to sign the beta `.app` so macOS Gatekeeper accepts it without "damaged" warnings.
...
3. Fill: User Email = your Apple ID, Common Name = `Argus Beta CSR`, leave CA Email empty.
...
2. Right-click → **Export 2 items…** → format **Personal Information Exchange (.p12)** → save as `argus-beta-developer-id.p12` somewhere private.
3. Set a strong export password. **Save it in 1Password** as `Argus Beta — APPLE_CERTIFICATE_PASSWORD`.
4. Convert to base64 + put on clipboard:

   ```sh
   base64 -i argus-beta-developer-id.p12 | pbcopy
   ```

   Save in 1Password as `Argus Beta — APPLE_CERTIFICATE`.

### 1.4 Note your Team ID

From [developer.apple.com/account](https://developer.apple.com/account) (top-right). 10-character string like `ABC123DE45`. Save in 1Password as `Argus Beta — APPLE_TEAM_ID`.
...
5. Save in 1Password as `Argus Beta — APPLE_PASSWORD`. Also record your Apple ID email as `Argus Beta — APPLE_ID`.
...
3. **Create bucket** → name `argus-beta-releases` → location: Automatic → Create.

### 3.2 Enable public access

We are not using a custom domain (yet). The bucket exposes a `pub-<hash>.r2.dev` URL.

1. In the new bucket, go to **Settings**.
2. Under **Public access** → **R2.dev subdomain** → click **Allow Access**. Confirm the dev-mode caveat.
3. Cloudflare will assign a URL like `https://pub-9f3a1b7e.r2.dev`. **Copy this.** Save in 1Password as `Argus Beta — R2_PUBLIC_URL`.

> The `r2.dev` URL is rate-limited (Cloudflare warns it's "for development"). For ~5 team members it's plenty. When we go to public release we'll add a custom domain — that's a one-line change in `tauri.beta.conf.json` plus republishing.

### 3.3 Create an API token scoped to the bucket

1. Go to **R2 → Manage R2 API Tokens** (top-right of the R2 dashboard).
2. **Create API token**.
3. Token name: `argus-beta-ci`.
4. Permissions: **Object Read & Write**.
5. Specify bucket: `argus-beta-releases` (NOT all buckets).
6. TTL: Forever (or set a calendar reminder for rotation).
7. Create. Cloudflare shows **Access Key ID** and **Secret Access Key**.

Save in 1Password:

- `Argus Beta — R2_ACCESS_KEY_ID`
- `Argus Beta — R2_SECRET_ACCESS_KEY`

### 3.4 Note your account ID

From the R2 dashboard, top-right (or **Account Home → Account ID**). Looks like `1a2b3c4d5e6f7g8h9i0j`. Save as `Argus Beta — R2_ACCOUNT_ID`.
...
```sh
mkdir -p ~/.tauri
pnpm tauri signer generate -- -w ~/.tauri/argus-beta.key
```
...
- The **private key** is written to `~/.tauri/argus-beta.key`.

### 4.2 Save in 1Password (CRITICAL)

Save four entries:
- `Argus Beta — TAURI_UPDATER_PUBLIC_KEY` (paste the public key string).
- `Argus Beta — TAURI_UPDATER_PRIVATE_KEY` (paste the **contents** of `~/.tauri/argus-beta.key`, i.e. `cat ~/.tauri/argus-beta.key`).
- `Argus Beta — TAURI_UPDATER_KEY_PASSWORD` (the passphrase).
...
### 4.3 Put the public key in tauri.beta.conf.json

Open `src-tauri/tauri.beta.conf.json` and replace the `pubkey` placeholder with the public key value from step 4.1. The full URL `https://pub-<hash>.r2.dev/latest.json` (with R2_PUBLIC_URL from step 3.2) goes into the `endpoints` array.
...
| `R2_BUCKET` | `argus-beta-releases` | step 3.1 |
| `R2_PUBLIC_URL` | `https://pub-<hash>.r2.dev` | step 3.2 |
| `TAURI_UPDATER_PRIVATE_KEY` | contents of `~/.tauri/argus-beta.key` | step 4.2 |
...
The beta build uses `src-tauri/icons-beta/` (parallel to `src-tauri/icons/`). Exact same dimensions, but visually distinct so it's recognizable in the dock alongside a future production build.

Approach: take the production icon, apply an orange tint or add a small orange badge in the lower-right corner. The exact treatment is design-side; the only requirement is "differentiable at a glance without zoom."

Files needed (match `src-tauri/icons/` names and sizes exactly):
- `src-tauri/icons-beta/32x32.png`
- `src-tauri/icons-beta/128x128.png`
- `src-tauri/icons-beta/128x128@2x.png`
- `src-tauri/icons-beta/icon.icns`
- `src-tauri/icons-beta/icon.ico`

Generate the `.icns` from PNGs via:
```sh
mkdir argus-beta.iconset
# place 16x16, 32x32, 64x64, 128x128, 256x256, 512x512, 1024x1024 PNGs inside
# (named per Apple's iconset convention: icon_16x16.png, icon_16x16@2x.png, etc.)
iconutil -c icns argus-beta.iconset -o icon.icns
```

Once placed, do a sanity build (`pnpm tauri build --config src-tauri/tauri.beta.conf.json`) and visually confirm the resulting `.app` shows the new icon.

---

## 7. Allow GitHub Actions to commit + tag
...
4. Download `https://pub-<hash>.r2.dev/Argus_Beta_0.1.1_aarch64.dmg` (or `_x64.dmg` on Intel macs), install it, launch.
...
3. Update `pubkey` in `tauri.beta.conf.json` and merge.
...
1. Go to **R2 → argus-beta-releases → Objects**.
...
4. Edit the `platforms.darwin-aarch64.url` and `darwin-x86_64.url` to point at the still-uploaded artifacts of that version (e.g. `Argus_Beta_0.1.5_aarch64.app.tar.gz`).
5. Edit the `signature` fields to match those previous archives. **The signatures are stored in the `.sig` files alongside the archives.** Download `Argus_Beta_0.1.5_aarch64.app.tar.gz.sig` and copy its contents into the `signature` field. Same for x86_64.
...
1. Quit Argus Beta.
2. Open `~/Library/Application Support/com.argus.beta.app/` and clear nothing — data persists across versions.
...
**App opens but never auto-updates** — open the log: `~/Library/Logs/Argus Beta/argus.log`. Common causes: endpoint URL wrong (typo in `R2_PUBLIC_URL`), pubkey mismatch (the build embedded a different pubkey than what signs the manifest now — check that the value in `tauri.beta.conf.json` matches the current `TAURI_UPDATER_PRIVATE_KEY`), R2 returning 404 for `latest.json` (the upload step may have failed silently — check the workflow run).

**`r2.dev` URL throws 1015 / rate limit** — usually transient. If sustained, you've hit Cloudflare's free-tier dev URL caps. Sign up for a custom domain on Cloudflare (free), bind to the R2 bucket, replace the URL in `tauri.beta.conf.json`. The team's apps will continue using the old URL until they auto-update to a build that has the new URL — to force migration, manually distribute one fresh `.dmg`.
