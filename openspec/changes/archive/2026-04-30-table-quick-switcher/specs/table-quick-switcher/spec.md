...
The switcher SHALL list, as searchable entries, every relation of kind `table`, `view`, or `materialized-view` that is present in the in-memory schema cache for any currently active Postgres connection. Relations from connections that are not currently active MUST NOT appear. When a connection becomes active or inactive, or when the cache receives new relations, the visible list MUST update reactively without requiring the user to close and reopen the switcher.
