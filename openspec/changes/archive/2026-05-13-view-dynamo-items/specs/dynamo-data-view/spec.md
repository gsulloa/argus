...
#### Scenario: Complex-type cell renders as summary

- **WHEN** a Tabla cell holds the AttributeValue `{ "L": [1, 2, 3] }`
- **THEN** the cell renders `[3 items]` and clicking it selects the row and focuses the inspector on that attribute
...
#### Scenario: Sets render but do not edit

- **WHEN** the selected item has an attribute `tags: { "SS": ["a", "b", "c"] }`
- **THEN** the inspector shows `tags : SS : ["a", "b", "c"]` (read-only)

#### Scenario: Escape clears selection

- **WHEN** a row is selected and the user presses Escape with focus inside the data view tab
