...
- **First save (no `state.saved_query_id`)**: open a modal `SaveAsModal` with a single field — `Name` (text input, required, pre-filled with the tab title if non-default). The modal MUST NOT prompt the user to choose a destination folder: the query is always saved into the current connection's linked context folder. On confirm:
