# cloudwatch-insights-ai-chat Specification

## Purpose

Brings the docked AI chat experience to the CloudWatch Logs Insights editor: a
collapsible, resizable `ChatPanel` driven by the tab's connection, with readiness
that is context-folder-optional (ready on a configured provider alone). Generated
CloudWatch Logs Insights queries can be applied or inserted into the editor, and a
completed Insights run can be attached as read-only context for the next message.
