...
`mssql_run_sql` and each step of `mssql_run_sql_many` SHALL cap the returned row set at the
...
`Dialect::TSql` result of `explicit_row_limit`: a leading `SELECT [ALL|DISTINCT] TOP (<int>)` /
`TOP <int>` without `PERCENT`, or a trailing depth-0 `OFFSET <int> ROWS FETCH NEXT <int> ROWS ONLY`
(the `FETCH NEXT` value).
...
further rows from the server (drop the tiberius row stream / send Attention to cancel further
fetches). The cap is per-statement, not per-run. The backend MUST fetch up to `effective_cap + 1` rows and set
`truncated = (fetched > effective_cap)`, so a result landing exactly on the cap is reported
complete — see `sql-result-row-cap`, "Exactly-at-cap results are not truncated". The `query_ms` MUST measure end-to-end including
the time spent fetching the cap-many rows.

An explicit `TOP` / `FETCH NEXT` in the statement MUST NOT be overridden downward: the effective
cap is never lower than the statement's own limit up to `HARD_ROW_CAP`.
...
- **WHEN** a SELECT against a 1M-row table runs without TOP / OFFSET ... FETCH and `sql.rowCap` is 10,000
...
#### Scenario: Explicit TOP above the configured cap is honoured

- **WHEN** the user runs `SELECT TOP (30000) * FROM dbo.big_table` against a table with more than 30,000 rows, with `sql.rowCap` at 10,000
- **THEN** the response has `rows.length === 30000`, `truncated: false`, and `row_cap: 30000`

#### Scenario: Explicit OFFSET/FETCH NEXT above the configured cap is honoured

- **WHEN** the user runs `SELECT * FROM dbo.big_table ORDER BY id OFFSET 0 ROWS FETCH NEXT 25000 ROWS ONLY`
- **THEN** the effective cap is 25,000 and the response is not truncated by the client

#### Scenario: TOP PERCENT does not raise the cap

- **WHEN** the user runs `SELECT TOP 50 PERCENT * FROM dbo.big_table` with `sql.rowCap` at 10,000, matching more than 10,000 rows
- **THEN** the response has `rows.length === 10000`, `truncated: true`, and `row_cap: 10000`
...
- Display the shared `<TruncationBanner />` above the grid whenever the response has `truncated: true`, per the `sql-result-row-cap` capability. The banner MUST name the response's `row_cap` rather than a hardcoded `10,000`, and its dialect clause is `TOP / OFFSET … FETCH NEXT`.
...
- **THEN** a banner reads `Showing the first 10,000 rows — the result hit Argus's row limit. Raise the limit or add a TOP / OFFSET … FETCH NEXT clause.` above the grid
...
### Requirement: MSSQL run responses carry the effective row cap

`mssql_run_sql` and each `outcome: "ok"` step of `mssql_run_sql_many` SHALL carry two additional
...
For MS SQL Server the engine ceiling is `HARD_ROW_CAP`, so `row_cap_source` is never `"engine"`.
...
- **WHEN** `mssql_run_sql` returns a rows result with `sql.rowCap` at `10000` and no statement limit
