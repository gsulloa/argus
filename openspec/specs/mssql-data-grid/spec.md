...
When the MSSQL grid is in editable mode, opening a cell's inline editor and leaving it without changing the content MUST NOT create a dirty buffer entry, and the cell MUST NOT render with the dirty-state (yellow) background. "Leaving
without a change" covers every editor-exit path that commits — blur, `Enter`, and
`Tab` — and every editor variant (text, numeric, JSON, boolean select). The dirty
highlight MUST be reserved for cells whose value the user actually changed, so an
edited cell is always visually distinguishable from a cell that was only
double-clicked.

This behavior MUST hold regardless of the column's data type. Because the inline
editor coerces its raw input into a typed `EditValue` on commit (e.g. a
`DECIMAL`/`NUMERIC` column whose server value is the string `"100.00"` is coerced
to the number `100`), a naive `JSON.stringify` comparison between the server value
and the coerced commit value can differ even when the user changed nothing. The
implementation MUST prevent a spurious dirty entry in these cases — either by not
committing when the editor content is unchanged from the value it opened with, or
by making the shared edit buffer's drop-if-equals-original collapse tolerant of
numeric string↔number round-trips (or both).

#### Scenario: Double-click then click away on a text cell stays clean

- **WHEN** the user double-clicks an editable `NVARCHAR` cell (opening the inline editor) and then clicks another cell without typing
- **THEN** the cell does NOT render with the dirty-state background
- **AND** the edit buffer contains no entry for that cell

#### Scenario: Double-click then commit an unchanged decimal cell stays clean

- **WHEN** a `DECIMAL` cell's server value is `"100.00"` and the user double-clicks it and presses `Enter` (or `Tab`, or blurs) without editing the text
- **THEN** the cell does NOT render with the dirty-state background
...
### Requirement: MSSQL table viewer tab acquires keyboard focus on activation

When an MSSQL table viewer tab becomes the active tab — either because it just mounted as the active tab (opened from the schema tree, the table quick switcher, or any other entry point) or because the user switched to it from another tab — the viewer SHALL move keyboard focus into itself, so that its root-scoped shortcuts (`⌘R` reload, `⌘Z` undo, `⌘S` save) and every data-grid key binding (`⌘C` copy, `⌘V` row paste, `⌘A` select-all, `Backspace`/`Delete`, `Escape`) are live from the first keystroke, with no priming click. As in MySQL, these shortcuts are bound to a React `onKeyDown` on the tab root, which receives nothing while focus sits on `document.body`.
...
The fallback MUST be selected by verifying that focus actually landed inside the tab root, NOT by testing whether a grid reference exists: the viewer keeps the grid mounted under an inline `display: none` while a non-Data subtab is showing, and focusing a non-rendered element is a silent no-op.
...
#### Scenario: Opening an MSSQL table focuses the grid

- **WHEN** the user activates a table node and an MSSQL table viewer tab opens as the active tab
...
- **WHEN** an MSSQL table tab has just been opened, rows are loaded, and the user clicks a single cell then presses `⌘A`
- **THEN** every loaded row is selected and a subsequent `⌘C` copies the selection

#### Scenario: Focus does not change selection

- **WHEN** an MSSQL table tab becomes active and focus lands on the grid root
...
- **WHEN** an MSSQL table tab becomes active while its first query is still in flight or it is showing an error state (the grid is unmounted), or while a non-Data subtab is showing (the grid is mounted but `display: none`)
...
- **WHEN** the user is typing in any input, textarea, select, or CodeMirror surface and an MSSQL table viewer tab activation would otherwise fire
...
- **WHEN** an active MSSQL table tab reloads its rows while the user has focus in the inspector or the filter bar
