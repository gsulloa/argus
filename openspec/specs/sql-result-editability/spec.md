# sql-result-editability Specification

## Purpose
Engine-neutral contract for deciding whether an ad-hoc SQL result set can be
written back: the `ResultEditability` payload shape, the provenance rules that
produce it, the closed reason taxonomy the UI renders, and the rule that a
pending edit is addressed by base column + primary key rather than by grid
position. Implemented for Postgres (`postgres-sql-editor`, resolved from the
wire's `RowDescription` provenance); written so MySQL/MSSQL can adopt the same
