...
When the MySQL grid is in editable mode, opening a cell's inline editor and leaving it without changing the content MUST NOT create a dirty buffer entry, and the cell MUST NOT render with the dirty-state (yellow) background. "Leaving
without a change" covers every editor-exit path that commits — blur, `Enter`, and
`Tab` — and every editor variant (text, numeric, JSON, boolean select, enum/set
editor). The dirty highlight MUST be reserved for cells whose value the user
...
editor coerces its raw input into a typed `EditValue` on commit (e.g. a `DECIMAL`
column whose server value is the string `"100.00"` is coerced to the number `100`;
a `JSON` column's text is re-canonicalized), a naive `JSON.stringify` comparison
...
the value it opened with, or by making the shared edit buffer's
drop-if-equals-original collapse tolerant of numeric string↔number and
canonicalized-JSON round-trips (or both).

#### Scenario: Double-click then click away on a text cell stays clean

- **WHEN** the user double-clicks an editable `VARCHAR` cell (opening the inline editor) and then clicks another cell without typing
