# home-onboarding-checklist Specification

## Purpose

Guide new users through initial app setup by surfacing a "Getting started"
checklist on the Welcome (home) tab. The checklist tracks three setup steps —
adding a connection, configuring an AI provider, and linking a context folder —
reflects their satisfied state reactively, provides direct calls-to-action into
the existing flows, and collapses once setup is complete so returning users are
not re-prompted.
